<script>
    ...
    import { geofietspuntencolors } from "../utils/geocolors.js"; // Zorg dat dit pad klopt
    ...
    onMount(async () => {
      if (typeof window !== "undefined") {
        ...
        try {
          const response = await fetch("../data/fietskruispunten_nieuwe_situatie.geojson"); // Pas dit pad aan indien nodig
          if (!response.ok) {
          ...
          function style(feature) {
            return { color: geofietspuntencolors(feature.properties.Layer), weight: 5 };
          }
